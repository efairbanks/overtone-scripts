...
(defn play-beat [clock tick]
  (do
    ...
    (play-rhythm-for-tick
     ...
     ctl
     [glitchinst
      :retrigmix (wrap-at (E 13 93) tick)
      :t-retrig 0
    ...
    (play-rhythm-for-tick
     (concat
      ...
      (E 3 8)
      (E 5 8))
     16 tick clock psamp [bd909 0.25 0.9 :outbus (wrap-at [fxbus glitchbus] tick)])
    (comment)
    (play-rhythm-for-tick
     (concat
      (E 5 8))
    ...
    (play-rhythm-for-tick
     (concat
      (E 11 20))
     10 tick clock buzz [(+ (rand 3) (midi->hz (xpose-note (first (first chds)) 30))) 0.2 :outbus glitchbus])
    (play-rhythm-for-tick
     (concat
      (E 11 20))
     10 tick clock buzz [(+ (rand 6) (midi->hz (xpose-note (second (first chds)) 58))) 0.07 :outbus glitchbus])
    (play-rhythm-for-tick
     (concat
      (E 5 8))
     4
